...
- Attendance and payroll are separated by role: every employee, including Admins, can submit work-time and leave requests; Managers can approve telecaller requests, and Admins can approve requests in their queue, set each employee's hourly rate, and calculate approved-hours pay in the private Payroll workspace. Logged calls are captured automatically as approved calling time; extra categories such as events, training, and admin work can be logged separately.
