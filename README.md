...
- A dedicated attendance worker automatically closes any forgotten workplace check-in at 12:00 AM in ATTENDANCE_TIMEZONE (India time by default). It records the actual local midnight as the checkout time and creates the same pending attendance time entry used by manual checkout, so the existing approval workflow remains intact.
