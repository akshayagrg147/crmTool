...
function ActivityChart({ volume, stale }: { volume: { hour: number; calls: number }[]; stale: boolean }) {
  const peak = volume.reduce((best, item) => item.calls > best.calls ? item : best, { hour: 0, calls: 0 });
  return <section className={`rounded-[18px] border border-ink-100 bg-surface p-5 shadow-card transition-opacity sm:p-6 ${stale ? "opacity-60" : ""}`}><SectionHeader eyebrow="Call activity" title="When your team is talking" detail="Hourly call volume across the selected performance range." action={<Link to="/analytics" className="btn-ghost px-2.5 py-1.5 text-xs">View report <ArrowRight size={13} /></Link>} />{!volume.length ? <EmptyState icon={PhoneCall} title="No call activity yet" message="Logged calls will form a visual activity pattern here." /> : <><div className="mt-5 h-52"><ResponsiveContainer width="100%" height="100%"><AreaChart data={volume} margin={{ top: 8, right: 4, left: -18, bottom: 0 }}><defs><linearGradient id="callVolumeFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2F6F6D" stopOpacity={0.28} /><stop offset="100%" stopColor="#2F6F6D" stopOpacity={0.02} /></linearGradient></defs><CartesianGrid vertical={false} stroke="#E7E8E6" strokeDasharray="4 4" /><XAxis dataKey="hour" tickFormatter={(hour) => `${hour}:00`} tick={{ fill: "#828D96", fontSize: 10 }} tickLine={false} axisLine={false} interval="preserveStartEnd" /><YAxis allowDecimals={false} tick={{ fill: "#9AA4AC", fontSize: 10 }} tickLine={false} axisLine={false} width={28} /><Tooltip labelFormatter={(hour) => `${hour}:00`} formatter={(value: number) => [value, "Calls"]} contentStyle={{ borderRadius: 10, border: "1px solid #E1E3E2", boxShadow: "0 14px 34px -22px rgba(24,37,51,.32)" }} /><Area type="monotone" dataKey="calls" stroke="#2F6F6D" strokeWidth={2.5} fill="url(#callVolumeFill)" dot={false} activeDot={{ r: 4, fill: "#B8893A", stroke: "#FFFFFF", strokeWidth: 2 }} /></AreaChart></ResponsiveContainer></div><div className="mt-3 flex items-center justify-between gap-3 border-t border-ink-100 pt-3 text-xs"><span className="flex items-center gap-2 text-ink-500"><span className="h-2 w-2 rounded-full bg-secondary" /> Peak hour <strong className="text-ink-800">{peak.calls ? `${peak.hour}:00` : "—"}</strong></span><span className="font-semibold text-ink-700">{volume.reduce((sum, item) => sum + item.calls, 0).toLocaleString("en-IN")} calls tracked</span></div></>}</section>;
}
